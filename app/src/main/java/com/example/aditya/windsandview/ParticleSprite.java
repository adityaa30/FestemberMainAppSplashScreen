...
public class ParticleSprite {
    ...
    public ParticleSprite(Bitmap particleBitmap, int particleCount, int x, int y, long fadeOutTimeMillisecond){
        mParticleBitmap = particleBitmap;
        mX = x;
        mY = y;
    ...
    public void draw(Canvas canvas){
        for (int i = 0; i < mParticleCount; i++) {
            update(i);
            canvas.drawBitmap(mParticleBitmap, x.get(i), y.get(i), mFadeOutPaint.get(i));
        }
    }
    ...
    private void update(int i){
        ...
        y.set(i, i*screenHeight/mParticleCount + yVelocity.get(i)*realTimeMillisecond /*+ yAcceleration.get(i)*realTimeMillisecond*realTimeMillisecond*/);
        mFadeOutPaint.get(i).setAlpha(255 - (int)((realTimeMillisecond + 0f)/(mFadeOutTime + 0f) * 255));
        //Log.v("ABCDEFG", 255 - ((realTimeMillisecond + 0f)/(mFadeOutTime + 0f) * 255) +"");
